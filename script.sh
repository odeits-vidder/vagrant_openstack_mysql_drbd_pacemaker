...
sudo sed -i -e 's/\#listen_tls = 0/listen_tls = 0/g' /etc/libvirt/libvirtd.conf
sudo sed -i -e 's/\#listen_tcp = 1/listen_tcp = 1/g' /etc/libvirt/libvirtd.conf
sudo sed -i -e 's/\#auth_tls = "none"/auth_tcp = "none"/g' /etc/libvirt/libvirtd.conf

sudo sed -i -e 's/env libvirtd_opts="-d"/env libvirtd_opts="-d -l"/g' /etc/init/libvirt-bin.conf 

sudo sed -i -e 's/libvirtd_opts="-d"/libvirtd_opts="-d -l"/g' /etc/default/libvirt-bin
